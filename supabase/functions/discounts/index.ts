...
function hasPermission(ctx: { permissions: string[] }, perm: string): boolean {
...
function mapDiscountError(ctx: EdgeRequestContext, msg: string): Response {
  if (msg.includes('INVOICE_NOT_FOUND'))    return err(ctx, 'Invoice not found', 404, 'NOT_FOUND');
  if (msg.includes('INVOICE_NOT_DRAFT'))    return err(ctx, 'Discounts can only be applied to draft invoices', 409, 'CONFLICT');
  if (msg.includes('DISCOUNT_NOT_FOUND'))   return err(ctx, 'Discount not found', 404, 'NOT_FOUND');
  if (msg.includes('DISCOUNT_INACTIVE'))    return err(ctx, 'Discount is not active', 422, 'DISCOUNT_INACTIVE');
  if (msg.includes('DISCOUNT_EXPIRED'))     return err(ctx, 'Discount has expired', 422, 'DISCOUNT_EXPIRED');
  if (msg.includes('DISCOUNT_ZERO'))        return err(ctx, 'Computed discount amount is zero', 422, 'DISCOUNT_ZERO');
  if (msg.includes('DISCOUNT_SCOPE_MISMATCH')) return err(ctx, 'Discount scope does not match invoice contents', 422, 'SCOPE_MISMATCH');
  if (msg.includes('COUPON_NOT_FOUND'))     return err(ctx, 'Coupon code not found', 404, 'NOT_FOUND');
  if (msg.includes('COUPON_INACTIVE'))      return err(ctx, 'Coupon code is not active', 422, 'COUPON_INACTIVE');
  if (msg.includes('COUPON_EXPIRED'))       return err(ctx, 'Coupon code has expired', 422, 'COUPON_EXPIRED');
  if (msg.includes('COUPON_LIMIT_REACHED')) return err(ctx, 'Coupon redemption limit reached', 422, 'COUPON_LIMIT');
  if (msg.includes('COUPON_STUDENT_LIMIT')) return err(ctx, 'Coupon already used the maximum times by this student', 422, 'STUDENT_LIMIT');
  if (msg.includes('PERIOD_LOCKED'))        return err(ctx, 'Financial period is locked', 423, 'PERIOD_LOCKED');
  return err(ctx, msg, 422, 'DISCOUNT_FAILED');
}
...
Deno.serve((req: Request) => serveCors(req, async () => {
  ...
  if (discountId === null && sub === 'redeem' && method === 'POST') {
    if (!hasPermission(ctx, 'finance:discount:assign')) return err(ctx, 'Forbidden', 403, 'FORBIDDEN');
    ...
    if (!invoice_id || !coupon_code || !student_id) {
      return err(ctx, 'invoice_id, coupon_code, student_id are required', 400, 'VALIDATION_ERROR');
    }
    ...
    if (rpcErr) return mapDiscountError(ctx, rpcErr.message as string);
  ...
  if (discountId !== null) {
    ...
    if (sub === 'apply' && method === 'POST') {
      ...
      let body: Record<string, unknown>;
      try { body = await req.json(); } catch { return err(ctx, 'Invalid JSON body', 400, 'VALIDATION_ERROR'); }
      if (!body['invoice_id']) return err(ctx, 'invoice_id is required', 400, 'VALIDATION_ERROR');
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (sub === 'deactivate' && method === 'POST') {
      if (!hasPermission(ctx, 'finance:discount:create')) return err(ctx, 'Forbidden', 403, 'FORBIDDEN');
      const { error: updateErr } = await client
        .from('discount_definitions')
        .update({ is_active: false, updated_by: ctx.actorId })
        .eq('id', discountId)
        .eq('organization_id', orgId);
      if (updateErr) return err(ctx, updateErr.message, 500, 'UPDATE_FAILED');
      return json({ id: discountId, is_active: false });
    }

    // GET /discounts/:id/coupons
    if (sub === 'coupons' && method === 'GET') {
      if (!hasPermission(ctx, 'finance:coupon:read')) return err(ctx, 'Forbidden', 403, 'FORBIDDEN');
    ...
    if (sub === 'coupons' && method === 'POST') {
      if (!hasPermission(ctx, 'finance:coupon:create')) return err(ctx, 'Forbidden', 403, 'FORBIDDEN');
      let body: Record<string, unknown>;
      try { body = await req.json(); } catch { return err(ctx, 'Invalid JSON body', 400, 'VALIDATION_ERROR'); }
      if (!body['code']) return err(ctx, 'code is required', 400, 'VALIDATION_ERROR');
      ...
      if (insertErr) {
        if (insertErr.message.includes('duplicate key')) {
          return err(ctx, 'Coupon code already exists', 409, 'DUPLICATE_CODE');
        }
        return err(ctx, insertErr.message, 500, 'INSERT_FAILED');
      }
    ...
    if (sub === null && method === 'GET') {
      if (!hasPermission(ctx, 'finance:discount:read')) return err(ctx, 'Forbidden', 403, 'FORBIDDEN');
      const { data, error: qErr } = await client
        .from('discount_definitions')
        .select('*')
        .eq('id', discountId)
        ...
        .maybeSingle();
      if (qErr) return err(ctx, qErr.message, 500, 'QUERY_FAILED');
      if (data === null) return err(ctx, 'Discount not found', 404, 'NOT_FOUND');
      return json(data);
    }
  ...
  if (method === 'POST') {
    ...
    if (!name || !discount_type || discount_value === undefined) {
      return err(ctx, 'name, discount_type, discount_value are required', 400, 'VALIDATION_ERROR');
    }
    const { data: discount, error: insertErr } = await client
      ...
      .select('*')
      .single();
    if (insertErr) return err(ctx, insertErr.message, 500, 'INSERT_FAILED');
