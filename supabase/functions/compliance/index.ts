...
async function handleGetEvents(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetAgiSubmissions(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:payroll:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url    = new URL(req.url);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGenerateAgi(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:payroll:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { agi_export_id } = body as { agi_export_id?: string };
  if (!agi_export_id || !UUID_RE.test(agi_export_id)) {
    return err('agi_export_id (UUID) is required', 400, 'INVALID_INPUT');
  }
  ...
  if (error) return err(error.message, 422, 'GENERATION_FAILED');
  return json({ submission_id: data }, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleCertifyAgi(submissionId: string, client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:payroll:manage')) return err('Forbidden', 403, 'FORBIDDEN');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleCreateAgiCorrection(submissionId: string, req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:payroll:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { correction_reason, description } = body as { correction_reason?: string; description?: string };
  if (!correction_reason || !description) {
    return err('correction_reason and description are required', 400, 'INVALID_INPUT');
  }
  ...
  if (error) return err(error.message, 422, 'CORRECTION_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetVatDeclarations(req: Request, client: any, orgId: string, user: any): Promise<Response> {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGenerateVat(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  ...
  if (!vat_period_id || !UUID_RE.test(vat_period_id)) {
    return err('vat_period_id (UUID) is required', 400, 'INVALID_INPUT');
  }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleCertifyVat(declarationId: string, client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:vat:manage')) return err('Forbidden', 403, 'FORBIDDEN');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleCreateVatCorrection(declarationId: string, req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:vat:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { correction_type, description } = body as { correction_type?: string; description?: string };
  if (!correction_type || !description) {
    return err('correction_type and description are required', 400, 'INVALID_INPUT');
  }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGenerateSaft(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { period_start, period_end, scope } = body as { period_start?: string; period_end?: string; scope?: string };
  if (!period_start || !period_end) {
    return err('period_start and period_end are required', 400, 'INVALID_INPUT');
  }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetSaftExports(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url    = new URL(req.url);
  const status = url.searchParams.get('status') ?? null;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidateFiling(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { filing_type, filing_id } = body as { filing_type?: string; filing_id?: string };
  if (!filing_type || !filing_id || !UUID_RE.test(filing_id)) {
    return err('filing_type and filing_id (UUID) are required', 400, 'INVALID_INPUT');
  }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetRetentionPolicies(client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const { data, error } = await client
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleEnforceRetention(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { policy_type, reference_date } = body as { policy_type?: string; reference_date?: string };
  if (!policy_type) {
    return err('policy_type is required', 400, 'INVALID_INPUT');
  }
  ...
  if (error) return err(error.message, 422, 'ENFORCEMENT_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetHashes(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url        = new URL(req.url);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetProfiles(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const { data, error } = await client
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetAssertions(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url    = new URL(req.url);
  const status = url.searchParams.get('status') ?? null;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleAssertDeterminism(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) {
    return err('entity_type and entity_id (UUID) are required', 400, 'INVALID_INPUT');
  }
  ...
  if (error) return err(error.message, 422, 'ASSERTION_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetRegistry(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url        = new URL(req.url);
  const exportType = url.searchParams.get('export_type') ?? null;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetSnapshots(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url        = new URL(req.url);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleCreateSnapshot(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidateSnapshot(req: Request, client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { snapshot_id } = body as { snapshot_id?: string };
  if (!snapshot_id || !UUID_RE.test(snapshot_id)) {
    return err('snapshot_id (UUID) is required', 400, 'INVALID_INPUT');
  }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleBuildPayload(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) {
  ...
  if (error) return err(error.message, 422, 'BUILD_FAILED');
  return json({ entity_type, entity_id, canonical_payload: data });
}

// Phase 5A.3: Canonical serialization validation suite
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidateSerialization(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleCertifyRegulatoryFiling(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGenerateFilingCertificate(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleVerifyFilingCertificate(req: Request, client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { certification_id } = body as { certification_id?: string };
  if (!certification_id || !UUID_RE.test(certification_id)) {
    return err('certification_id (UUID) is required', 400, 'INVALID_INPUT');
  }
  const { data, error } = await client.rpc('verify_filing_certificate', {
    p_certification_id: certification_id,
  });
  if (error) return err(error.message, 422, 'VERIFICATION_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetRegulatoryCertifications(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url        = new URL(req.url);
  const entityType = url.searchParams.get('entity_type') ?? null;
  const entityId   = url.searchParams.get('entity_id') ?? null;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleBuildEvidencePackage(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) {
  ...
  if (error) return err(error.message, 422, 'EVIDENCE_BUILD_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetEvidencePackages(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url        = new URL(req.url);
  const entityType = url.searchParams.get('entity_type') ?? null;
  const entityId   = url.searchParams.get('entity_id') ?? null;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleVerifyLineage(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) {
  ...
  if (error) return err(error.message, 422, 'LINEAGE_VERIFICATION_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetLineageRecords(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url        = new URL(req.url);
  const entityType = url.searchParams.get('entity_type') ?? null;
  const entityId   = url.searchParams.get('entity_id') ?? null;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGenerateReplayCertificate(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) {
  ...
  if (error) return err(error.message, 422, 'REPLAY_CERTIFICATE_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidatePhase5b(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleSignCertificate(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:write')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { cert_id, signing_key_id } = body;
  if (!cert_id || !signing_key_id) return err('cert_id and signing_key_id required', 400, 'MISSING_FIELDS');
  ...
  if (error) return err(error.message, 422, 'SIGN_CERTIFICATE_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleVerifySignature(req: Request, client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { signature_id } = body;
  if (!signature_id) return err('signature_id required', 400, 'MISSING_FIELDS');
  const { data, error } = await client.rpc('verify_certificate_signature', { p_signature_id: signature_id });
  if (error) return err(error.message, 422, 'VERIFY_SIGNATURE_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetSignatures(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url = new URL(req.url);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetSigningKeys(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const { data, error } = await client
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleRegisterReceipt(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:write')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  ...
  if (!entity_type || !entity_id || !envelope_id || !authority_name || !submission_hash)
    return err('entity_type, entity_id, envelope_id, authority_name, submission_hash required', 400, 'MISSING_FIELDS');
  ...
  if (error) return err(error.message, 422, 'REGISTER_RECEIPT_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleVerifyReceipt(req: Request, client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { receipt_id } = body;
  if (!receipt_id) return err('receipt_id required', 400, 'MISSING_FIELDS');
  const { data, error } = await client.rpc('verify_authority_receipt', { p_receipt_id: receipt_id });
  if (error) return err(error.message, 422, 'VERIFY_RECEIPT_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetReceipts(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url = new URL(req.url);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleBuildEnvelope(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:write')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { entity_type, entity_id } = body;
  if (!entity_type || !entity_id) return err('entity_type and entity_id required', 400, 'MISSING_FIELDS');
  ...
  if (error) return err(error.message, 422, 'BUILD_ENVELOPE_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleVerifyIntegrity(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { entity_type, entity_id } = body;
  if (!entity_type || !entity_id) return err('entity_type and entity_id required', 400, 'MISSING_FIELDS');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetEnvelopes(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url = new URL(req.url);
  const entityType = url.searchParams.get('entity_type');
  const entityId   = url.searchParams.get('entity_id');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidatePhase5c(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleRegisterEndpoint(req: Request, client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:write')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { endpoint_key, authority_name, protocol } = body;
  if (!endpoint_key || !authority_name || !protocol)
    return err('endpoint_key, authority_name, protocol required', 400, 'MISSING_FIELDS');
  ...
  if (error) return err(error.message, 422, 'REGISTER_ENDPOINT_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleVerifyEndpointTrust(req: Request, client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { endpoint_id } = body;
  if (!endpoint_id) return err('endpoint_id required', 400, 'MISSING_FIELDS');
  const { data, error } = await client.rpc('verify_endpoint_trust', { p_endpoint_id: endpoint_id });
  if (error) return err(error.message, 422, 'VERIFY_ENDPOINT_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetEndpoints(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const { data, error } = await client
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleBuildManifest(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:write')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { entity_type, entity_id, endpoint_id } = body;
  if (!entity_type || !entity_id || !endpoint_id)
    return err('entity_type, entity_id, endpoint_id required', 400, 'MISSING_FIELDS');
  ...
  if (error) return err(error.message, 422, 'BUILD_MANIFEST_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetManifests(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url = new URL(req.url);
  const entityType = url.searchParams.get('entity_type');
  const entityId   = url.searchParams.get('entity_id');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleCreateDelivery(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:write')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { entity_type, entity_id, transport_manifest_id } = body;
  if (!entity_type || !entity_id || !transport_manifest_id)
    return err('entity_type, entity_id, transport_manifest_id required', 400, 'MISSING_FIELDS');
  ...
  if (error) return err(error.message, 422, 'CREATE_DELIVERY_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleRegisterAttempt(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:write')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { delivery_id, outcome } = body;
  if (!delivery_id || !outcome) return err('delivery_id and outcome required', 400, 'MISSING_FIELDS');
  ...
  if (error) return err(error.message, 422, 'REGISTER_ATTEMPT_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleFinalizeDelivery(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:write')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { delivery_id } = body;
  if (!delivery_id) return err('delivery_id required', 400, 'MISSING_FIELDS');
  const { data, error } = await client.rpc('finalize_regulatory_delivery', {
    p_org_id:      orgId,
    p_delivery_id: delivery_id,
    p_actor_id:    user.id,
  });
  if (error) return err(error.message, 422, 'FINALIZE_DELIVERY_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleVerifyDeliveryIntegrity(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  const { entity_type, entity_id } = body;
  if (!entity_type || !entity_id) return err('entity_type and entity_id required', 400, 'MISSING_FIELDS');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetDeliveries(req: Request, client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url = new URL(req.url);
  const entityType = url.searchParams.get('entity_type');
  const entityId   = url.searchParams.get('entity_id');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidatePhase5d(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
async function handleRegisterTrustAnchor(client: any, req: Request): Promise<Response> {
  ...
  if (error) return err(error.message, 422, 'PKI_ANCHOR_FAILED');
  return json({ anchor: data }, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetTrustAnchors(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
async function handleRegisterCertificateChain(client: any, req: Request): Promise<Response> {
  ...
  if (error) return err(error.message, 422, 'PKI_CHAIN_FAILED');
  return json({ chain: data }, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetCertificateChains(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
async function handleValidateCertificateChain(client: any, req: Request): Promise<Response> {
  const body = await req.json();
  if (!body.chain_id) return err('chain_id required', 400, 'MISSING_PARAM');
  const { data, error } = await client.rpc('validate_certificate_chain', { p_chain_id: body.chain_id });
  if (error) return err(error.message, 422, 'CHAIN_VALIDATION_FAILED');
  return json({ validation: data });
}
...
async function handleVerifyRevocationStatus(client: any, req: Request): Promise<Response> {
  ...
  if (error) return err(error.message, 422, 'REVOCATION_CHECK_FAILED');
...
async function handleRegisterSignedReceipt(client: any, req: Request, orgId: string): Promise<Response> {
  const body = await req.json();
  if (!body.authority_receipt_id || !body.detached_signature) return err('authority_receipt_id and detached_signature required', 400, 'MISSING_PARAM');
  ...
  if (error) return err(error.message, 422, 'SIGNED_RECEIPT_FAILED');
  return json({ signed_receipt: data }, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetSignedReceipts(client: any, user: any, orgId: string): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
async function handleVerifyAuthoritySignature(client: any, req: Request): Promise<Response> {
  const body = await req.json();
  if (!body.signed_receipt_id) return err('signed_receipt_id required', 400, 'MISSING_PARAM');
  const { data, error } = await client.rpc('verify_authority_signature', { p_signed_receipt_id: body.signed_receipt_id });
  if (error) return err(error.message, 422, 'SIGNATURE_VERIFY_FAILED');
...
async function handleVerifyTransportAuthenticity(client: any, req: Request, orgId: string): Promise<Response> {
  const body = await req.json();
  if (!body.entity_type || !body.entity_id) return err('entity_type and entity_id required', 400, 'MISSING_PARAM');
  const { data, error } = await client.rpc('verify_transport_authenticity', {
    p_org_id:      orgId,
    p_entity_type: body.entity_type,
    p_entity_id:   body.entity_id,
  });
  if (error) return err(error.message, 422, 'AUTHENTICITY_VERIFY_FAILED');
  return json({ authenticity: data });
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidatePhase5e(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
async function handleRegisterTimestampAuthority(client: any, req: Request): Promise<Response> {
  ...
  if (error) return err(error.message, 422, 'TSA_REGISTER_FAILED');
  return json({ authority: data }, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetTimestampAuthorities(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
async function handleIssueTimestampEvidence(client: any, req: Request, orgId: string): Promise<Response> {
  ...
  if (!body.entity_type || !body.entity_id || !body.authority_id || !body.timestamp_value || !body.timestamp_signature) {
    return err('entity_type, entity_id, authority_id, timestamp_value, timestamp_signature required', 400, 'MISSING_PARAM');
  }
  ...
  if (error) return err(error.message, 422, 'EVIDENCE_ISSUE_FAILED');
  return json({ evidence: data }, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetTemporalEvidence(client: any, user: any, orgId: string, req: Request): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
async function handleVerifyTimestampSignature(client: any, req: Request): Promise<Response> {
  const body = await req.json();
  if (!body.evidence_id) return err('evidence_id required', 400, 'MISSING_PARAM');
...
async function handleVerifyTemporalNonrepudiation(client: any, req: Request): Promise<Response> {
  ...
  if (error) return err(error.message, 422, 'NONREP_VERIFY_FAILED');
  return json({ verification: data });
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetChronologyLineage(client: any, user: any, orgId: string, req: Request): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const entityId = new URL(req.url).searchParams.get('entity_id');
...
async function handleVerifyChainIntegrity(client: any, req: Request, orgId: string): Promise<Response> {
  ...
  if (error) return err(error.message, 422, 'CHAIN_INTEGRITY_FAILED');
...
async function handleCreateTemporalSnapshot(client: any, req: Request, orgId: string): Promise<Response> {
  const body = await req.json();
  if (!body.entity_type || !body.entity_id || !body.at_timestamp) return err('entity_type, entity_id, at_timestamp required', 400, 'MISSING_PARAM');
  ...
  if (error) return err(error.message, 422, 'SNAPSHOT_CREATE_FAILED');
  return json({ snapshot: data }, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetTemporalSnapshots(client: any, user: any, orgId: string, req: Request): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const entityId = new URL(req.url).searchParams.get('entity_id');
...
async function handleGenerateTemporalReplayCertificate(client: any, req: Request, orgId: string): Promise<Response> {
  ...
  if (error) return err(error.message, 422, 'REPLAY_CERT_FAILED');
  return json({ replay_certificate: data }, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetReplayValidationSnapshots(client: any, user: any, orgId: string, req: Request): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const entityId = new URL(req.url).searchParams.get('entity_id');
...
async function handleValidateCertAtTimestamp(client: any, req: Request): Promise<Response> {
  const body = await req.json();
  if (!body.chain_id || !body.at_timestamp) return err('chain_id and at_timestamp required', 400, 'MISSING_PARAM');
  const { data, error } = await client.rpc('validate_certificate_at_timestamp', {
    p_chain_id:     body.chain_id,
    p_at_timestamp: body.at_timestamp,
  });
  if (error) return err(error.message, 422, 'CERT_VALIDATE_FAILED');
  return json({ validation: data });
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidatePhase5f(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
async function handleRegisterSerializerProfile(client: any, req: Request): Promise<Response> {
  ...
  if (!serializer_key || !serializer_version || !canonicalization_strategy || !introduced_phase) {
    return err('serializer_key, serializer_version, canonicalization_strategy, introduced_phase are required', 400, 'INVALID_INPUT');
  }
  ...
  if (error) return err(error.message, 422, 'REGISTER_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetSerializerRegistry(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const { data, error } = await client
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidateSerializerCompatibility(client: any, req: Request, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { serializer_key, check_chronology, check_evidence, check_trust } = body as Record<string, unknown>;
  if (!serializer_key) return err('serializer_key is required', 400, 'INVALID_INPUT');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleVerifySerializerReplay(client: any, req: Request, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleReconstructSerializerVersion(client: any, req: Request, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  ...
  if (!serializer_key || !serializer_version || !canonicalization_strategy) {
    return err('serializer_key, serializer_version, canonicalization_strategy are required', 400, 'INVALID_INPUT');
  }
  ...
  if (error) return err(error.message, 422, 'RECONSTRUCTION_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleAssertTemporalSecurityContext(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const { data, error } = await client.rpc('assert_temporal_security_context', {
    p_org_id:   orgId,
    p_actor_id: user.id,
  });
  if (error) return err(error.message, 403, 'SECURITY_CONTEXT_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleCreateReplayRangeWindow(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  ...
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id) || !window_start || !window_end) {
    return err('entity_type, entity_id (UUID), window_start, window_end are required', 400, 'INVALID_INPUT');
  }
  ...
  if (error) return err(error.message, 422, 'RANGE_WINDOW_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetReplayRangeWindows(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url      = new URL(req.url);
  const entityId = url.searchParams.get('entity_id') ?? null;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handlePrepareChronologyArchive(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  ...
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id as string) || start_seq === undefined || end_seq === undefined) {
    return err('entity_type, entity_id (UUID), start_seq, end_seq are required', 400, 'INVALID_INPUT');
  }
  ...
  if (error) return err(error.message, 422, 'ARCHIVE_PREP_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetChronologyArchives(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url      = new URL(req.url);
  const entityId = url.searchParams.get('entity_id') ?? null;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidatePhase5fAudit(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleRunReplayTest(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id, test_type } = body as { entity_type?: string; entity_id?: string; test_type?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) return err('entity_type and entity_id required', 400, 'INVALID_INPUT');
  const { data, error } = await client.rpc('run_replay_test', {
    p_org_id: orgId, p_entity_type: entity_type, p_entity_id: entity_id,
    p_test_type: test_type ?? 'full_chronology', p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'REPLAY_TEST_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleRunFullReplayReconstruction(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id, at_timestamp } = body as { entity_type?: string; entity_id?: string; at_timestamp?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id) || !at_timestamp) return err('entity_type, entity_id, at_timestamp required', 400, 'INVALID_INPUT');
  const { data, error } = await client.rpc('run_full_replay_reconstruction', {
    p_org_id: orgId, p_entity_type: entity_type, p_entity_id: entity_id,
    p_at_timestamp: at_timestamp, p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'RECONSTRUCTION_FAILED');
  return json(data, 201);
}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidateReplayDeterminism(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  ...
  const { data, error } = await client.rpc('validate_replay_determinism', {
    p_org_id: orgId, p_entity_type: entity_type, p_entity_id: entity_id,
    p_at_timestamp: at_timestamp, p_iterations: iterations ?? 2, p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'DETERMINISM_CHECK_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleCompareReplayRuns(client: any, req: Request, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { run_id_1, run_id_2 } = body as { run_id_1?: string; run_id_2?: string };
  if (!run_id_1 || !UUID_RE.test(run_id_1) || !run_id_2 || !UUID_RE.test(run_id_2)) return err('run_id_1 and run_id_2 required', 400, 'INVALID_INPUT');
  const { data, error } = await client.rpc('compare_replay_runs', { p_run_id_1: run_id_1, p_run_id_2: run_id_2 });
  if (error) return err(error.message, 422, 'COMPARE_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGenerateReplayReproducibilityReport(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { run_id_1, run_id_2 } = body as { run_id_1?: string; run_id_2?: string };
  if (!run_id_1 || !UUID_RE.test(run_id_1) || !run_id_2 || !UUID_RE.test(run_id_2)) return err('run_id_1 and run_id_2 required', 400, 'INVALID_INPUT');
  const { data, error } = await client.rpc('generate_replay_reproducibility_report', {
    p_org_id: orgId, p_run_id_1: run_id_1, p_run_id_2: run_id_2, p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'REPORT_FAILED');
  return json(data, 201);
}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleDetectSerializerDrift(client: any, req: Request, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const url = new URL(req.url);
  const serializerKey = url.searchParams.get('serializer_key') ?? '';
  if (!serializerKey) return err('serializer_key required', 400, 'INVALID_INPUT');
  const { data, error } = await client.rpc('detect_serializer_drift', { p_serializer_key: serializerKey });
  if (error) return err(error.message, 422, 'DRIFT_DETECTION_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGenerateSerializerDriftReport(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { serializer_key } = body as { serializer_key?: string };
  if (!serializer_key) return err('serializer_key required', 400, 'INVALID_INPUT');
  const { data, error } = await client.rpc('generate_serializer_drift_report', {
    p_org_id: orgId, p_serializer_key: serializer_key, p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'REPORT_FAILED');
  return json(data, 201);
}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGetSchemaHashIntegrity(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleBenchmarkReplayEngine(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { scale_factor } = body as { scale_factor?: number };
  const { data, error } = await client.rpc('benchmark_replay_engine', {
    p_org_id: orgId, p_scale_factor: scale_factor ?? 1, p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'BENCHMARK_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGenerateReplayPerformanceReport(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { benchmark_type } = body as { benchmark_type?: string };
  const { data, error } = await client.rpc('generate_replay_performance_report', {
    p_org_id: orgId, p_benchmark_type: benchmark_type ?? null, p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'REPORT_FAILED');
  return json(data, 201);
}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidateReplayAfterRestore(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id, pre_restore_hash } = body as { entity_type?: string; entity_id?: string; pre_restore_hash?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id) || !pre_restore_hash) return err('entity_type, entity_id, pre_restore_hash required', 400, 'INVALID_INPUT');
  const { data, error } = await client.rpc('validate_replay_after_restore', {
    p_org_id: orgId, p_entity_type: entity_type, p_entity_id: entity_id,
    p_pre_restore_hash: pre_restore_hash, p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'RESTORE_VALIDATION_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidateTemporalChainAfterRestore(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) return err('entity_type and entity_id required', 400, 'INVALID_INPUT');
  const { data, error } = await client.rpc('validate_temporal_chain_after_restore', {
    p_org_id: orgId, p_entity_type: entity_type, p_entity_id: entity_id, p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'CHAIN_VALIDATION_FAILED');
  return json(data, 201);
}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidateRestoreReproducibility(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { pre_backup_hash, post_restore_hash } = body as { pre_backup_hash?: string; post_restore_hash?: string };
  if (!pre_backup_hash || !post_restore_hash) return err('pre_backup_hash and post_restore_hash required', 400, 'INVALID_INPUT');
  const { data, error } = await client.rpc('validate_restore_reproducibility', {
    p_org_id: orgId, p_pre_backup_hash: pre_backup_hash, p_post_restore_hash: post_restore_hash, p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'REPRODUCIBILITY_CHECK_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGenerateRestoreIntegrityReport(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) return err('entity_type and entity_id required', 400, 'INVALID_INPUT');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidateTenantReplayIsolation(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) return err('entity_type and entity_id required', 400, 'INVALID_INPUT');
  ...
  if (error) return err(error.message, 422, 'ISOLATION_CHECK_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleGenerateTenantIsolationReport(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) return err('entity_type and entity_id required', 400, 'INVALID_INPUT');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleRunReplayHealthCheck(client: any, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
  const { data, error } = await client.rpc('run_replay_health_check', { p_org_id: orgId, p_actor_id: user.id });
  if (error) return err(error.message, 422, 'HEALTH_CHECK_FAILED');
  return json(data);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidateChronologyIntegrity(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) return err('entity_type and entity_id required', 400, 'INVALID_INPUT');
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleDetectReplayChainCorruption(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id } = body as { entity_type?: string; entity_id?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id)) return err('entity_type and entity_id required', 400, 'INVALID_INPUT');
  ...
  if (error) return err(error.message, 422, 'CORRUPTION_DETECTION_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleDetectReplayHashDivergence(client: any, req: Request, orgId: string, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:manage')) return err('Forbidden', 403, 'FORBIDDEN');
  const body = await req.json().catch(() => ({}));
  const { entity_type, entity_id, baseline_hash } = body as { entity_type?: string; entity_id?: string; baseline_hash?: string };
  if (!entity_type || !entity_id || !UUID_RE.test(entity_id) || !baseline_hash) return err('entity_type, entity_id, baseline_hash required', 400, 'INVALID_INPUT');
  const { data, error } = await client.rpc('detect_replay_hash_divergence', {
    p_org_id: orgId, p_entity_type: entity_type, p_entity_id: entity_id,
    p_baseline_hash: baseline_hash, p_actor_id: user.id,
  });
  if (error) return err(error.message, 422, 'DIVERGENCE_DETECTION_FAILED');
  return json(data, 201);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function handleValidatePhase6a(client: any, user: any): Promise<Response> {
  if (!hasPermission(user, 'finance:compliance:read')) return err('Forbidden', 403, 'FORBIDDEN');
...
Deno.serve((req: Request) => serveCors(req, async () => {
  ...
  if (seg1 === 'events' && method === 'GET') {
    return handleGetEvents(req, client, orgId, user);
  }

  // AGI routes
  if (seg1 === 'agi') {
    if (seg2 === 'submissions' && method === 'GET') {
      return handleGetAgiSubmissions(req, client, orgId, user);
    }
    if (seg2 === 'generate' && method === 'POST') {
      return handleGenerateAgi(req, client, orgId, user);
    }
    if (seg2 === 'certify' && id3 !== null && method === 'POST') {
      return handleCertifyAgi(id3, client, user);
    }
    if (seg2 === 'correct' && id3 !== null && method === 'POST') {
      return handleCreateAgiCorrection(id3, req, client, orgId, user);
    }
  }
  ...
  if (seg1 === 'vat') {
    if (seg2 === 'declarations' && method === 'GET') {
      return handleGetVatDeclarations(req, client, orgId, user);
    }
    if (seg2 === 'generate' && method === 'POST') {
      return handleGenerateVat(req, client, orgId, user);
    }
    if (seg2 === 'certify' && id3 !== null && method === 'POST') {
      return handleCertifyVat(id3, client, user);
    }
    if (seg2 === 'correct' && id3 !== null && method === 'POST') {
      return handleCreateVatCorrection(id3, req, client, orgId, user);
    }
  }
  ...
  if (seg1 === 'saft') {
    if (seg2 === 'generate' && method === 'POST') {
      return handleGenerateSaft(req, client, orgId, user);
    }
    if (seg2 === 'exports' && method === 'GET') {
      return handleGetSaftExports(req, client, orgId, user);
    }
  }

  // Filing validation
  if (seg1 === 'filings' && seg2 === 'validate' && method === 'POST') {
    return handleValidateFiling(req, client, orgId, user);
  }

  // Retention
  if (seg1 === 'retention') {
    if (seg2 === 'policies' && method === 'GET') {
      return handleGetRetentionPolicies(client, orgId, user);
    }
    if (seg2 === 'enforce' && method === 'POST') {
      return handleEnforceRetention(req, client, orgId, user);
    }
  }

  // Regulatory export hashes
  if (seg1 === 'hashes' && method === 'GET') {
    return handleGetHashes(req, client, orgId, user);
  }

  // Phase 5A.1: Canonicalization profiles
  if (seg1 === 'profiles' && method === 'GET') {
    return handleGetProfiles(client, user);
  }

  // Phase 5A.1: Replay assertions
  if (seg1 === 'assertions') {
    if (seg2 === '' && method === 'GET') return handleGetAssertions(req, client, orgId, user);
    if (seg2 === 'determinism' && method === 'POST') return handleAssertDeterminism(req, client, orgId, user);
  }

  // Phase 5A.1: Deterministic export registry
  if (seg1 === 'registry' && method === 'GET') {
    return handleGetRegistry(req, client, orgId, user);
  }

  // Phase 5A.1: Certification snapshots
  if (seg1 === 'snapshots') {
    if (seg2 === '' && method === 'GET')      return handleGetSnapshots(req, client, orgId, user);
    if (seg2 === '' && method === 'POST')     return handleCreateSnapshot(req, client, orgId, user);
    if (seg2 === 'validate' && method === 'POST') return handleValidateSnapshot(req, client, user);
  }

  // Phase 5A.2: Canonical payload builder
  if (seg1 === 'payload' && seg2 === 'build' && method === 'POST') {
    return handleBuildPayload(req, client, orgId, user);
  }

  // Phase 5A.3: Canonical serialization validation suite
  if (seg1 === 'validate' && seg2 === 'serialization' && method === 'GET') {
    return handleValidateSerialization(client, user);
  }

  // Phase 5B: Filing Certification & Regulatory Sealing
  if (seg1 === 'certifications') {
    if (seg2 === 'certify'      && method === 'POST') return handleCertifyRegulatoryFiling(req, client, orgId, user);
    if (seg2 === 'certificate'  && method === 'POST') return handleGenerateFilingCertificate(req, client, orgId, user);
    if (seg2 === 'verify'       && method === 'POST') return handleVerifyFilingCertificate(req, client, user);
    if (seg2 === ''             && method === 'GET')  return handleGetRegulatoryCertifications(req, client, orgId, user);
  }

  if (seg1 === 'evidence') {
    if (seg2 === 'build'  && method === 'POST') return handleBuildEvidencePackage(req, client, orgId, user);
    if (seg2 === ''       && method === 'GET')  return handleGetEvidencePackages(req, client, orgId, user);
  }

  if (seg1 === 'lineage') {
    if (seg2 === 'verify' && method === 'POST') return handleVerifyLineage(req, client, orgId, user);
    if (seg2 === ''       && method === 'GET')  return handleGetLineageRecords(req, client, orgId, user);
  }

  if (seg1 === 'replay' && seg2 === 'certify' && method === 'POST') {
    return handleGenerateReplayCertificate(req, client, orgId, user);
  }

  if (seg1 === 'validate' && seg2 === 'phase5b' && method === 'GET') {
    return handleValidatePhase5b(client, user);
  }

  // Phase 5C routing
  if (seg1 === 'signatures') {
    if (seg2 === 'sign'   && method === 'POST') return handleSignCertificate(req, client, orgId, user);
    if (seg2 === 'verify' && method === 'POST') return handleVerifySignature(req, client, user);
    if (seg2 === ''       && method === 'GET')  return handleGetSignatures(req, client, orgId, user);
  }

  if (seg1 === 'keys' && seg2 === '' && method === 'GET') {
    return handleGetSigningKeys(client, user);
  }

  if (seg1 === 'receipts') {
    if (seg2 === 'register' && method === 'POST') return handleRegisterReceipt(req, client, orgId, user);
    if (seg2 === 'verify'   && method === 'POST') return handleVerifyReceipt(req, client, user);
    if (seg2 === ''         && method === 'GET')  return handleGetReceipts(req, client, orgId, user);
  }

  if (seg1 === 'envelopes') {
    if (seg2 === 'build'     && method === 'POST') return handleBuildEnvelope(req, client, orgId, user);
    if (seg2 === 'integrity' && method === 'POST') return handleVerifyIntegrity(req, client, orgId, user);
    if (seg2 === ''          && method === 'GET')  return handleGetEnvelopes(req, client, orgId, user);
  }

  if (seg1 === 'validate' && seg2 === 'phase5c' && method === 'GET') {
    return handleValidatePhase5c(client, user);
  }

  // Phase 5D routing
  if (seg1 === 'endpoints') {
    if (seg2 === 'register' && method === 'POST') return handleRegisterEndpoint(req, client, user);
    if (seg2 === 'verify'   && method === 'POST') return handleVerifyEndpointTrust(req, client, user);
    if (seg2 === ''         && method === 'GET')  return handleGetEndpoints(client, user);
  }

  if (seg1 === 'manifests') {
    if (seg2 === 'build' && method === 'POST') return handleBuildManifest(req, client, orgId, user);
    if (seg2 === ''      && method === 'GET')  return handleGetManifests(req, client, orgId, user);
  }

  if (seg1 === 'deliveries') {
    if (seg2 === 'create'    && method === 'POST') return handleCreateDelivery(req, client, orgId, user);
    if (seg2 === 'attempt'   && method === 'POST') return handleRegisterAttempt(req, client, orgId, user);
    if (seg2 === 'finalize'  && method === 'POST') return handleFinalizeDelivery(req, client, orgId, user);
    if (seg2 === 'integrity' && method === 'POST') return handleVerifyDeliveryIntegrity(req, client, orgId, user);
    if (seg2 === ''          && method === 'GET')  return handleGetDeliveries(req, client, orgId, user);
  }

  if (seg1 === 'validate' && seg2 === 'phase5d' && method === 'GET') {
    return handleValidatePhase5d(client, user);
  }
  ...
  if (seg1 === 'pki') {
    ...
    if (seg2 === 'anchors' && seg3 === '' && method === 'GET') {
      return handleGetTrustAnchors(client, user);
    }
    // Certificate chains
    ...
    if (seg2 === 'chains' && seg3 === '' && method === 'GET') {
      return handleGetCertificateChains(client, user);
    }
    ...
    if (seg2 === 'signed-receipts' && seg3 === '' && method === 'GET') {
      return handleGetSignedReceipts(client, user, orgId);
    }
  ...
  if (seg1 === 'validate' && seg2 === 'phase5e' && method === 'GET') {
    return handleValidatePhase5e(client, user);
  }
  ...
  if (seg1 === 'temporal') {
    ...
    if (seg2 === 'authorities' && seg3 === '' && method === 'GET') {
      return handleGetTimestampAuthorities(client, user);
    }
    // Temporal evidence
    ...
    if (seg2 === 'evidence' && seg3 === '' && method === 'GET') {
      return handleGetTemporalEvidence(client, user, orgId, req);
    }
    ...
    if (seg2 === 'chronology' && seg3 === '' && method === 'GET') {
      return handleGetChronologyLineage(client, user, orgId, req);
    }
    ...
    if (seg2 === 'snapshots' && seg3 === '' && method === 'GET') {
      return handleGetTemporalSnapshots(client, user, orgId, req);
    }
    // Replay certificates
    ...
    if (seg2 === 'replay' && seg3 === '' && method === 'GET') {
      return handleGetReplayValidationSnapshots(client, user, orgId, req);
    }
    ...
    if (seg2 === 'serializers' && seg3 === '' && method === 'GET') {
      return handleGetSerializerRegistry(client, user);
    }
    if (seg2 === 'serializers' && seg3 === 'validate-compatibility' && method === 'POST') {
      return handleValidateSerializerCompatibility(client, req, user);
    }
    if (seg2 === 'serializers' && seg3 === 'verify-replay' && method === 'POST') {
      return handleVerifySerializerReplay(client, req, user);
    }
    if (seg2 === 'serializers' && seg3 === 'reconstruct' && method === 'POST') {
      return handleReconstructSerializerVersion(client, req, user);
    }
    // Phase 5F-Audit: Security context
    if (seg2 === 'security' && seg3 === 'assert' && method === 'POST') {
      return handleAssertTemporalSecurityContext(client, req, orgId, user);
    }
    // Phase 5F-Audit: Replay range windows (extended replay sub-path)
    if (seg2 === 'replay' && seg3 === 'range-window' && method === 'POST') {
      return handleCreateReplayRangeWindow(client, req, orgId, user);
    }
    if (seg2 === 'replay' && seg3 === 'range-windows' && method === 'GET') {
      return handleGetReplayRangeWindows(client, req, orgId, user);
    }
    // Phase 5F-Audit: Chronology archives (extended chronology sub-path)
    if (seg2 === 'chronology' && seg3 === 'archive' && method === 'POST') {
      return handlePrepareChronologyArchive(client, req, orgId, user);
    }
    if (seg2 === 'chronology' && seg3 === 'archives' && method === 'GET') {
      return handleGetChronologyArchives(client, req, orgId, user);
    }
  }

  if (seg1 === 'validate' && seg2 === 'phase5f' && method === 'GET') {
    return handleValidatePhase5f(client, user);
  }

  if (seg1 === 'validate' && seg2 === 'phase5f-audit' && method === 'GET') {
    return handleValidatePhase5fAudit(client, user);
  }

  // Phase 6A: Replay Test Harness routes (seg1 = 'harness')
  if (seg1 === 'harness') {
    if (seg2 === 'run-test' && method === 'POST') {
      return handleRunReplayTest(client, req, orgId, user);
    }
    if (seg2 === 'reconstruct' && method === 'POST') {
      return handleRunFullReplayReconstruction(client, req, orgId, user);
    }
    if (seg2 === 'validate-determinism' && method === 'POST') {
      return handleValidateReplayDeterminism(client, req, orgId, user);
    }
    if (seg2 === 'compare-runs' && method === 'POST') {
      return handleCompareReplayRuns(client, req, user);
    }
    if (seg2 === 'reproducibility-report' && method === 'POST') {
      return handleGenerateReplayReproducibilityReport(client, req, orgId, user);
    }
  }
  ...
  if (seg1 === 'drift') {
    if (seg2 === 'detect' && method === 'GET') {
      return handleDetectSerializerDrift(client, req, user);
    }
    if (seg2 === 'report' && method === 'POST') {
      return handleGenerateSerializerDriftReport(client, req, orgId, user);
    }
    if (seg2 === 'schema-integrity' && method === 'GET') {
      return handleGetSchemaHashIntegrity(client, user);
    }
  }
  ...
  if (seg1 === 'benchmark') {
    if (seg2 === 'replay-engine' && method === 'POST') {
      return handleBenchmarkReplayEngine(client, req, orgId, user);
    }
    if (seg2 === 'performance-report' && method === 'POST') {
      return handleGenerateReplayPerformanceReport(client, req, orgId, user);
    }
  }
  ...
  if (seg1 === 'restore') {
    if (seg2 === 'validate' && method === 'POST') {
      return handleValidateReplayAfterRestore(client, req, orgId, user);
    }
    if (seg2 === 'chain-validate' && method === 'POST') {
      return handleValidateTemporalChainAfterRestore(client, req, orgId, user);
    }
    if (seg2 === 'reproducibility' && method === 'POST') {
      return handleValidateRestoreReproducibility(client, req, orgId, user);
    }
    if (seg2 === 'integrity-report' && method === 'POST') {
      return handleGenerateRestoreIntegrityReport(client, req, orgId, user);
    }
  }
  ...
  if (seg1 === 'isolation') {
    if (seg2 === 'validate' && method === 'POST') {
      return handleValidateTenantReplayIsolation(client, req, orgId, user);
    }
    if (seg2 === 'report' && method === 'POST') {
      return handleGenerateTenantIsolationReport(client, req, orgId, user);
    }
  }
  ...
  if (seg1 === 'health') {
    if (seg2 === 'check' && method === 'GET') {
      return handleRunReplayHealthCheck(client, orgId, user);
    }
    if (seg2 === 'chronology-integrity' && method === 'POST') {
      return handleValidateChronologyIntegrity(client, req, orgId, user);
    }
    if (seg2 === 'detect-corruption' && method === 'POST') {
      return handleDetectReplayChainCorruption(client, req, orgId, user);
    }
    if (seg2 === 'detect-drift' && method === 'POST') {
      return handleDetectReplayHashDivergence(client, req, orgId, user);
    }
  }

  // Phase 6A: Validation Suite route
  if (seg1 === 'validate' && seg2 === 'phase6a' && method === 'GET') {
    return handleValidatePhase6a(client, user);
  }
  ...
  if (seg1 === 'ci') {
    if (seg2 === 'run-pipeline' && method === 'POST') {
      ...
      const { data, error } = await client.rpc('run_replay_ci_pipeline', {
        p_org_id: orgId, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'CI_PIPELINE_ERROR');
      return ok(data);
    }
    if (seg2 === 'validate-migration' && method === 'POST') {
      ...
      const { data, error } = await client.rpc('validate_migration_reproducibility', {
        ...
        p_pre_hash: body.pre_hash, p_post_hash: body.post_hash, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'MIGRATION_REPRO_ERROR');
      return ok(data);
    }
    if (seg2 === 'post-deploy-integrity' && method === 'POST') {
      const body = await req.json();
      const { data, error } = await client.rpc('verify_post_deploy_replay_integrity', {
        p_org_id: orgId, p_deploy_ver: body.deploy_ver, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'DEPLOY_INTEGRITY_ERROR');
      return ok(data);
    }
    if (seg2 === 'smoke-tests' && method === 'POST') {
      const body = await req.json();
      const { data, error } = await client.rpc('execute_replay_smoke_tests', {
        p_org_id: orgId, p_run_id: body.run_id, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'SMOKE_TESTS_ERROR');
      return ok(data);
    }
    if (seg2 === 'deployment-report' && method === 'POST') {
      ...
      if (error) return err(error.message, 500, 'DEPLOYMENT_REPORT_ERROR');
      return ok(data);
    }
  }
  ...
  if (seg1 === 'rebuild') {
    if (seg2 === 'run' && method === 'POST') {
      ...
      const { data, error } = await client.rpc('run_shadow_rebuild_validation', {
        p_org_id: orgId, p_rebuild_ver: body.rebuild_ver, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'SHADOW_REBUILD_ERROR');
      return ok(data);
    }
    if (seg2 === 'compare' && method === 'POST') {
      ...
      if (error) return err(error.message, 500, 'SHADOW_COMPARE_ERROR');
      return ok(data);
    }
    if (seg2 === 'validate-equivalence' && method === 'POST') {
      ...
      const { data, error } = await client.rpc('validate_shadow_replay_equivalence', {
        ...
        p_shadow_hash: body.shadow_hash, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'SHADOW_EQUIV_ERROR');
      return ok(data);
    }
    if (seg2 === 'detect-divergence' && method === 'POST') {
      ...
      if (error) return err(error.message, 500, 'REBUILD_DIVERGENCE_ERROR');
      return ok(data);
    }
    if (seg2 === 'report' && method === 'POST') {
      const body = await req.json();
      ...
      if (error) return err(error.message, 500, 'SHADOW_REPORT_ERROR');
      return ok(data);
    }
  }
  ...
  if (seg1 === 'simulation') {
    if (seg2 === 'cold-restore' && method === 'POST') {
      ...
      const { data, error } = await client.rpc('simulate_cold_restore_validation', {
        p_org_id: orgId, p_sim_ver: body.sim_ver, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'COLD_RESTORE_ERROR');
      return ok(data);
    }
    if (seg2 === 'equivalence' && method === 'POST') {
      ...
      const { data, error } = await client.rpc('validate_restore_replay_equivalence', {
        ...
        p_post_hash: body.post_hash, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'RESTORE_EQUIV_ERROR');
      return ok(data);
    }
    if (seg2 === 'compare-hashes' && method === 'POST') {
      ...
      if (error) return err(error.message, 500, 'RESTORE_COMPARE_ERROR');
      return ok(data);
    }
    if (seg2 === 'benchmark' && method === 'POST') {
      ...
      if (error) return err(error.message, 500, 'RESTORE_BENCHMARK_ERROR');
      return ok(data);
    }
    if (seg2 === 'report' && method === 'POST') {
      const body = await req.json();
      ...
      if (error) return err(error.message, 500, 'RESTORE_REPORT_ERROR');
      return ok(data);
    }
  }
  ...
  if (seg1 === 'archive') {
    if (seg2 === 'create-batch' && method === 'POST') {
      ...
      if (error) return err(error.message, 500, 'ARCHIVE_BATCH_ERROR');
      return ok(data);
    }
    if (seg2 === 'validate-integrity' && method === 'POST') {
      const body = await req.json();
      const { data, error } = await client.rpc('validate_archive_replay_integrity', {
        p_org_id: orgId, p_batch_id: body.batch_id, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'ARCHIVE_INTEGRITY_ERROR');
      return ok(data);
    }
    if (seg2 === 'hash-continuity' && method === 'GET') {
      ...
      if (error) return err(error.message, 500, 'ARCHIVE_CONTINUITY_ERROR');
      return ok(data);
    }
    if (seg2 === 'reconstruct' && method === 'POST') {
      const body = await req.json();
      ...
      if (error) return err(error.message, 500, 'ARCHIVE_RECONSTRUCT_ERROR');
      return ok(data);
    }
    if (seg2 === 'report' && method === 'GET') {
      ...
      if (error) return err(error.message, 500, 'ARCHIVE_REPORT_ERROR');
      return ok(data);
    }
  }
  ...
  if (seg1 === 'observe') {
    if (seg2 === 'collect-metrics' && method === 'POST') {
      ...
      if (error) return err(error.message, 500, 'METRICS_COLLECT_ERROR');
      return ok(data);
    }
    if (seg2 === 'growth-rate' && method === 'GET') {
      ...
      if (error) return err(error.message, 500, 'GROWTH_RATE_ERROR');
      return ok(data);
    }
    if (seg2 === 'detect-anomalies' && method === 'POST') {
      ...
      if (error) return err(error.message, 500, 'INTEGRITY_ANOMALY_ERROR');
      return ok(data);
    }
    if (seg2 === 'health' && method === 'GET') {
      ...
      if (error) return err(error.message, 500, 'OPERATIONAL_HEALTH_ERROR');
      return ok(data);
    }
    if (seg2 === 'report' && method === 'GET') {
      ...
      if (error) return err(error.message, 500, 'OPERABILITY_REPORT_ERROR');
      return ok(data);
    }
  }
  ...
  if (seg1 === 'anomaly') {
    if (seg2 === 'detect' && method === 'POST') {
      ...
      const { data, error } = await client.rpc('detect_replay_anomalies', {
        p_org_id: orgId, p_entity_type: body.entity_type,
        p_entity_id: body.entity_id ?? null, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'ANOMALY_DETECT_ERROR');
      return ok(data);
    }
    if (seg2 === 'discontinuities' && method === 'POST') {
      ...
      if (error) return err(error.message, 500, 'DISCONTINUITY_ERROR');
      return ok(data);
    }
    if (seg2 === 'chain-integrity' && method === 'POST') {
      ...
      if (error) return err(error.message, 500, 'CHAIN_INTEGRITY_ERROR');
      return ok(data);
    }
    if (seg2 === 'serializer-divergence' && method === 'POST') {
      ...
      const { data, error } = await client.rpc('detect_serializer_divergence', {
        ...
        p_expected_hash: body.expected_hash, p_actor_id: user.id,
      });
      if (error) return err(error.message, 500, 'SERIALIZER_DIVERGENCE_ERROR');
      return ok(data);
    }
    if (seg2 === 'report' && method === 'POST') {
      const body = await req.json();
      ...
      if (error) return err(error.message, 500, 'ANOMALY_REPORT_ERROR');
      return ok(data);
    }
  }
  ...
  if (seg1 === 'validate' && seg2 === 'phase6b' && method === 'GET') {
    ...
    if (error) return err(error.message, 500, 'PHASE6B_VALIDATION_ERROR');
    return ok(data);
  }
