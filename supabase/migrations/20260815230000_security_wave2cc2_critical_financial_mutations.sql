-- SECURITY REMEDIATION WAVE 2C-C2 — CRITICAL FINANCIAL MUTATION
-- AUTHORIZATION HARDENING
--
-- Fixes 12 functions: create_order, the 4 GL-posting functions
-- (post_retained_earnings_entry, post_year_end_profit_transfer,
-- post_impairment_adjustment, post_opening_balance_entry), and the 7
-- fiscal-year/accrual/deferred-revenue/fixed-asset functions Wave 2B
-- itself flagged as deferred and never actually remediated
-- (create_fiscal_year, assign_period_to_fiscal_year,
-- create_accrual_schedule, cancel_accrual_schedule,
-- create_periodic_deferred_schedule, register_fixed_asset,
-- rollover_opening_balances).
--
-- All 12 were re-verified this wave to have real, permission-gated,
-- authenticated Edge Function callers (orders/index.ts,
-- enrollments/index.ts, financial-close/index.ts, payroll/index.ts,
-- accruals/index.ts, fixed-assets/index.ts) that server-derive both
-- organization and actor from trusted context — never from the client
-- request body. The vulnerability closed here is exclusively the direct
-- RPC bypass path (skipping the Edge Function's permission gate and
-- server-side org/actor derivation entirely).
--
-- Fix pattern (identical to every prior wave): direct p_org_id/
-- p_organization_id parameters get an early guard against
-- public.auth_organization_id() (Pattern A); entity-derived functions
-- with no org parameter get a merged not-found/wrong-org exception
-- reusing the function's own existing message (Pattern B); functions that
-- already had a partial entity-to-entity consistency check
-- (post_year_end_profit_transfer, assign_period_to_fiscal_year,
-- rollover_opening_balances) get the missing caller-vs-derived-org check
-- added on top of what already existed. Actor checks added wherever
-- p_actor_id is accepted. public.is_platform_admin() and
-- public.is_trusted_service_context() (both reused unchanged from Wave
-- 2B) bypass throughout — no new authorization mechanism.
--
-- post_year_end_profit_transfer additionally gets a genuinely new check:
-- neither p_new_period_id nor p_prior_period_id was ever verified against
-- p_org_id before this migration — both are now confirmed to belong to
-- the org before the function proceeds.
--
-- register_fixed_asset's p_asset_class_id requires no tenant check:
-- fixed_asset_classes has no organization_id column (confirmed via
-- information_schema) — it is shared BAS asset-class taxonomy, not
-- tenant data.
--
-- WAVE 2A LESSON reapplied: every REVOKE below explicitly includes
-- PUBLIC, anon, and authenticated together; every grant is re-verified
-- live via has_function_privilege() after applying this migration.
...
-- (Wave 2A lesson). All 12 functions have real authenticated-role callers
-- (orders/index.ts, enrollments/index.ts, financial-close/index.ts,
-- payroll/index.ts, accruals/index.ts, fixed-assets/index.ts — all
-- confirmed using anon-key + forwarded Authorization header).
-- ============================================================================

REVOKE EXECUTE ON FUNCTION public.create_order(uuid, uuid, uuid, uuid, text, text, text, integer, uuid, text, uuid, text, numeric, numeric, numeric, numeric, text, text, uuid, text, jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_order(uuid, uuid, uuid, uuid, text, text, text, integer, uuid, text, uuid, text, numeric, numeric, numeric, numeric, text, text, uuid, text, jsonb) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.post_retained_earnings_entry(uuid, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.post_retained_earnings_entry(uuid, uuid) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.post_year_end_profit_transfer(uuid, uuid, uuid, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.post_year_end_profit_transfer(uuid, uuid, uuid, uuid) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.post_impairment_adjustment(uuid, uuid, date, numeric, text, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.post_impairment_adjustment(uuid, uuid, date, numeric, text, uuid) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.post_opening_balance_entry(uuid, uuid, jsonb, text, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.post_opening_balance_entry(uuid, uuid, jsonb, text, uuid) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.create_fiscal_year(uuid, integer, date, date, text, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_fiscal_year(uuid, integer, date, date, text, uuid) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.assign_period_to_fiscal_year(uuid, uuid, boolean, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.assign_period_to_fiscal_year(uuid, uuid, boolean, uuid) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.create_accrual_schedule(uuid, uuid, accrual_type, text, numeric, date, integer, text, text, text, text, text, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_accrual_schedule(uuid, uuid, accrual_type, text, numeric, date, integer, text, text, text, text, text, uuid) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.cancel_accrual_schedule(uuid, text, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.cancel_accrual_schedule(uuid, text, uuid) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.create_periodic_deferred_schedule(uuid, uuid, text, uuid, text, numeric, date, integer, text, text, text, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_periodic_deferred_schedule(uuid, uuid, text, uuid, text, numeric, date, integer, text, text, text, uuid) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.register_fixed_asset(uuid, uuid, uuid, text, text, date, numeric, numeric, integer, depreciation_method, text, text, text, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.register_fixed_asset(uuid, uuid, uuid, text, text, date, numeric, numeric, integer, depreciation_method, text, text, text, uuid) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.rollover_opening_balances(uuid, uuid, uuid) FROM PUBLIC, anon, authenticated;
