-- ════════════════════════════════════════════════════════════════════════════
-- Refund Processed — Notification Templates + Rules
--
-- event-worker's handleRefundProcessed() has been a
-- `// TODO: enqueue in accounting_export_queue; notify student of refund`
-- no-op since it was written — a student who receives a refund is currently
-- never told. This migration adds the missing system templates + rules,
-- following the exact same pattern as 20260727000009's reservation_expired
-- completion; the event-worker code change (Communication.Requested enqueue)
-- is deployed alongside this migration. (The ledger side of this same TODO
-- was closed separately via post_refund_journal_entry — 20260724000010 —
-- and its frontend wiring; this migration closes the notification side.)
-- ════════════════════════════════════════════════════════════════════════════
...
(NULL, 'refund.processed', 'sv', 'sms',
 NULL,
 'Hej {förnamn}, en återbetalning på {belopp} kr har behandlats för faktura {fakturanummer}. Pengarna når ditt konto inom några bankdagar.',
 ARRAY['förnamn','belopp','fakturanummer'],
 true),

(NULL, 'refund.processed', 'sv', 'email',
 'Din återbetalning har behandlats',
 E'Hej {{förnamn}},\n\nEn återbetalning på {{belopp}} kr har behandlats för faktura {{fakturanummer}}.\n\nPengarna når ditt konto inom några bankdagar.\n\nMed vänliga hälsningar\n{{school_name}}',
 ARRAY['förnamn','belopp','fakturanummer','school_name'],
 true),

(NULL, 'refund.processed', 'sv', 'push',
...
-- ── 2. Extend seed_org_communication() to wire refund_processed rules ─────────
...
-- 20260622000003/20260723000001/20260727000009, extended with one more event.
...
-- ── 3. Backfill refund_processed notification_rules for existing orgs ─────────
