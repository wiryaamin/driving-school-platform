...
-- All indexes use IF NOT EXISTS for idempotent replay (governance standard I-1).
--
-- Dependencies:
...
CREATE INDEX IF NOT EXISTS accounting_export_entries_tx_date_idx
...
CREATE INDEX IF NOT EXISTS accounting_export_entries_account_debit_idx
  ON public.accounting_export_entries (organization_id, account_debit);

CREATE INDEX IF NOT EXISTS accounting_export_entries_account_credit_idx
...
CREATE INDEX IF NOT EXISTS accounting_export_queue_unmapped_idx
...
CREATE INDEX IF NOT EXISTS invoices_org_outstanding_status_idx
...
CREATE INDEX IF NOT EXISTS invoices_org_issued_at_idx
...
CREATE INDEX IF NOT EXISTS invoices_org_overdue_idx
...
CREATE INDEX IF NOT EXISTS payments_org_status_confirmed_idx
  ON public.payments (organization_id)
  WHERE status = 'confirmed';

-- payment_reconciliation_summary — total_allocated aggregate by org
CREATE INDEX IF NOT EXISTS payment_allocations_org_idx
...
CREATE INDEX IF NOT EXISTS refunds_org_completed_processed_idx
  ON public.refunds (organization_id, processed_at)
  WHERE refund_status = 'completed';

-- refund_metrics_summary — group by reason_code
CREATE INDEX IF NOT EXISTS refunds_org_reason_code_idx
...
CREATE INDEX IF NOT EXISTS refunds_invoice_id_completed_idx
...
CREATE INDEX IF NOT EXISTS credit_balance_cache_org_balance_idx
  ON public.credit_balance_cache (organization_id)
  WHERE balance > 0;

-- accounting_export_queue pending count (dashboard snapshot)
CREATE INDEX IF NOT EXISTS accounting_export_queue_org_pending_idx
  ON public.accounting_export_queue (organization_id)
  WHERE exported_at IS NULL;

-- finance_vat_summary_v / vat_liability_summary — issued month grouping
-- issued_at is timestamptz; date_trunc(text, timestamptz) is STABLE (not IMMUTABLE).
-- AT TIME ZONE 'UTC' converts timestamptz → timestamp (no timezone info);
-- date_trunc(text, timestamp) IS IMMUTABLE — safe for expression indexes.
-- UTC buckets match Stockholm calendar months for all business-hours invoice data.
CREATE INDEX IF NOT EXISTS invoices_org_issued_month_vat_idx
  ON public.invoices (organization_id, (DATE_TRUNC('month', (issued_at AT TIME ZONE 'UTC'))::date))
  WHERE issued_at IS NOT NULL
...
CREATE INDEX IF NOT EXISTS accounting_export_runs_org_format_completed_idx
