...
export function SysteminställningarPage() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      toast({ title: 'Sparat', description: 'Systeminställningarna har sparats.' });
  ...
  return (
    <div className="max-w-xl space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        ...
        <Toggle checked={bankId} onChange={setBankId} label="Mobilt BankID för personal" desc="Tillåt personalen att logga in med Mobilt BankID istället för e-post och lösenord. Kräver att personnummer är korrekt registrerat." />
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        ...
        <label className="flex items-start gap-2.5 cursor-pointer">
          <input type="checkbox" checked={crossDeptSearch} onChange={e => setCrossDeptSearch(e.target.checked)} className="rounded border-border accent-primary w-4 h-4 mt-0.5 shrink-0" />
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            <input type="number" value={lockDays} onChange={e => setLockDays(Number(e.target.value))} min={0} max={365} className="w-20 h-9 px-3 text-sm border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40 text-center" />
            <span className="text-sm text-muted-foreground">dagar</span>
          </div>
        </div>
        <label className="flex items-start gap-2.5 cursor-pointer">
          <input type="checkbox" checked={allowPayDate} onChange={e => setAllowPayDate(e.target.checked)} className="rounded border-border accent-primary w-4 h-4 mt-0.5 shrink-0" />
          <span className="text-sm text-foreground">Kan ställa in betalningsdatum vid registrering av betalning</span>
        </label>
        <div className="space-y-1">
          <label className="text-xs font-medium text-foreground">Användare som kan redigera pris, rabatter, anställdas timmar och kreditgräns:</label>
          ...
          <select value={editPriceRole} onChange={e => setEditPriceRole(e.target.value)} className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 text-foreground">
            {roleOpts.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
        </div>
        <div className="space-y-1">
          <label className="text-xs font-medium text-foreground">Användare som kan registrera krediteringar:</label>
          ...
          <select value={creditRole} onChange={e => setCreditRole(e.target.value)} className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 text-foreground">
            {roleOpts.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
        </div>
        <div className="flex justify-end"><SaveBtn /></div>
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        ...
        <div className="space-y-1">
          ...
          <input type="time" value={afternoonTime} onChange={e => setAfternoonTime(e.target.value)} className="h-9 px-3 text-sm border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40" />
        </div>
        <div className="flex justify-end"><SaveBtn /></div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        ...
        {logoFile ? (
          <div className="flex items-center gap-3 p-3 rounded-lg border border-border bg-muted/20">
            ...
            <button type="button" onClick={() => setLogoFile(null)} className="text-xs text-destructive hover:underline shrink-0">Ta bort</button>
          </div>
        ) : (
          <label className="cursor-pointer block">
            ...
            <input type="file" className="sr-only" accept="image/*" onChange={e => { const f = e.target.files?.[0]; if (f) setLogoFile(f); }} />
          </label>
        )}
        <div className="flex justify-end"><SaveBtn /></div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            <input type="number" value={creditLimit} onChange={e => setCreditLimit(Number(e.target.value))} min={0} className="w-28 h-9 px-3 text-sm border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40 text-center" />
            <span className="text-sm text-muted-foreground">kr</span>
          </div>
        </div>
        <label className="flex items-start gap-2.5 cursor-pointer">
          <input type="checkbox" checked={blockSalesOver} onChange={e => setBlockSalesOver(e.target.checked)} className="rounded border-border accent-primary w-4 h-4 mt-0.5 shrink-0" />
