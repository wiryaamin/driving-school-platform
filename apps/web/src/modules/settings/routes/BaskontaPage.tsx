...
function AccountSelect({ accounts, value, onChange }: { accounts: BasAccount[]; value: string; onChange: (v: string) => void }) {
  return (
    <select value={value} onChange={e => onChange(e.target.value)} className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 text-foreground">
...
function AccountRow({ label, accounts, value, onChange }: { label: string; accounts: BasAccount[]; value: string; onChange: (v: string) => void }) {
  return (
    <div className="flex items-center gap-4 py-2 border-b border-border/50 last:border-0">
      <span className="flex-1 text-sm text-foreground">{label}</span>
      <div className="w-52 shrink-0"><AccountSelect accounts={accounts} value={value} onChange={onChange} /></div>
    </div>
  );
}
...
const DEFAULTS: Record<string, string> = {
  kf_privat: '1510', ej_fakt_momsfri: '1790', ej_fakt_moms6: '1790', ej_fakt_moms12: '1790', ej_fakt_moms25: '1790',
  utg_moms6: '2630', utg_moms12: '2620', utg_moms25: '2610', ing_moms: '2641',
  forskott_momsfri: '2420', forskott_moms6: '2420', forskott_moms12: '2420', forskott_moms25: '2420', forskott_ospec: '2422',
  pay_bank: '1940', pay_kontant: '1910', pay_kort: '1930', pay_kreditkort: '1930', pay_swish: '1943',
  pay_bankgiro: '1940', pay_presentkort: '2421', pay_overforing_in: '1940', pay_overforing_ut: '1940',
  pay_tabs_swish: '1943', pay_tabs_nets: '1941', pay_billecta: '1930',
  ret_bank: '1940', ret_kontant: '1910', ret_swish: '1943', ret_kort: '1930',
  ret_presentkort: '2421', ret_tabs_swish: '1943', ret_tabs_nets: '1941', ret_bankgiro: '1940',
};
...
export function BaskontaPage() {
  ...
  useEffect(() => {
    ...
    const accounts_saved = (orgSettings['bas_accounts'] as Record<string, string> | undefined) ?? {};
    if (Object.keys(accounts_saved).length > 0) setVals(prev => ({ ...prev, ...accounts_saved }));
  }, [orgSettings]);
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      await supabase.from('organizations').update({
        settings: {
          ...(orgSettings ?? {}),
          bas_interval: { interval, bokfor_day: bokforDay, kost_stelle: kostStalle },
          bas_accounts: vals,
        },
      } as never).eq('id', orgId);
    },
    onSuccess: () => {
      toast({ title: 'Sparat', description: 'Baskontona har sparats.' });
  ...
  return (
    <div className="max-w-2xl space-y-4">
          ...
          <table className="w-full text-sm">
            <tbody className="divide-y divide-border">
              {[
                ...
                ['Nästa tidpunkt för generering av verifikat', (() => { const d = new Date(); d.setMonth(d.getMonth() + 1, bokforDay); return d.toISOString().slice(0, 10); })()],
              ].map(([label, value]) => (
                <tr key={label}><td className="px-4 py-2.5 text-foreground font-medium">{label}</td><td className="px-4 py-2.5 text-muted-foreground text-right">{value}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
      ...
      <SectionCard title="Tillgångar" colHeader="Kontonummer">
        {([['Kundfordringar','kf_privat'],['Ej fakturerat (momsbefriad)','ej_fakt_momsfri'],['Ej fakturerat (moms 6%)','ej_fakt_moms6'],['Ej fakturerat (moms 12%)','ej_fakt_moms12'],['Ej fakturerat (moms 25%)','ej_fakt_moms25']] as [string, string][]).map(([label, key]) => (
      ...
      <SectionCard title="Eget kapital och skulder" colHeader="Kontonummer">
        {([['Utgående moms 6%','utg_moms6'],['Utgående moms 12%','utg_moms12'],['Utgående moms 25%','utg_moms25'],['Ingående moms','ing_moms'],['Förskottsbetalning (momsbefriad)','forskott_momsfri'],['Förskottsbetalning (moms 6%)','forskott_moms6'],['Förskottsbetalning (moms 12%)','forskott_moms12'],['Förskottsbetalning (moms 25%)','forskott_moms25'],['Förskottsbetalning (ospecificerad)','forskott_ospec']] as [string, string][]).map(([label, key]) => (
          <AccountRow key={key} label={label} accounts={accounts} value={vals[key] ?? ''} onChange={set(key)} />
        ))}
      </SectionCard>

      <SectionCard title="Betalningar" colHeader="Kontonummer">
        {([['Bank','pay_bank'],['Kontant','pay_kontant'],['Kort','pay_kort'],['Kreditkort','pay_kreditkort'],['Swish','pay_swish'],['Bankgiro','pay_bankgiro'],['Astra presentkort','pay_presentkort'],['Överföring in','pay_overforing_in'],['Överföring ut','pay_overforing_ut'],['TABS Swish','pay_tabs_swish'],['TABS Nets','pay_tabs_nets'],['Billecta betalning','pay_billecta']] as [string, string][]).map(([label, key]) => (
          <AccountRow key={key} label={label} accounts={accounts} value={vals[key] ?? ''} onChange={set(key)} />
        ))}
      </SectionCard>

      <SectionCard title="Återbetalningar" colHeader="Kontonummer">
        {([['Återbetalning bank','ret_bank'],['Återbetalning kontant','ret_kontant'],['Återbetalning Swish','ret_swish'],['Återbetalning kort','ret_kort'],['Återbetalning Astra presentkort','ret_presentkort'],['Återbetalning TABS Swish','ret_tabs_swish'],['Återbetalning TABS Nets','ret_tabs_nets'],['Återbetalning Bankgiro','ret_bankgiro']] as [string, string][]).map(([label, key]) => (
          <AccountRow key={key} label={label} accounts={accounts} value={vals[key] ?? ''} onChange={set(key)} />
        ))}
      </SectionCard>

      <div className="flex justify-end">
        <Button onClick={save} disabled={isPending} className="bg-green-500 hover:bg-green-600 text-white">
          {isPending ? 'Sparar…' : 'Spara baskonton'}
        </Button>
      </div>
    </div>
  );
