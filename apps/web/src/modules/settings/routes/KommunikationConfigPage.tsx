...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 shrink-0 rounded-full border-2 border-transparent transition-colors ${checked ? 'bg-primary' : 'bg-input'}`}
    >
  ...
  return (
    <div className="max-w-xl space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-5">
        <CheckRow
          checked={smsDisabled}
          onChange={setSmsDisabled}
          label="Inaktivera SMS-påminnelse"
        />

        {!smsDisabled && (
          <>
            {/* Morning reminder sub-section */}
            <div className="space-y-3 pl-1">
              <h3 className="text-sm font-semibold text-foreground">
                SMS-påminnelse med morgondagens bokningar för elev
              </h3>
              <p className="text-xs text-muted-foreground leading-relaxed">
                Ange tid för att skicka SMS. Observera att ändringen inte träder i kraft
                förrän dagen efter.
              </p>
              ...
              <CheckRow
                checked={newStudentDefault}
                onChange={setNewStudentDefault}
                label="Nya elever ska ha en SMS-påminnelse som standard"
              />

              <button type="button" className="text-sm text-primary hover:underline">
                Klicka här för att se dagens SMS-påminnelse baserat på sparade inställningar
              </button>
            </div>

            {/* Message template sub-section */}
            <div className="space-y-2 pt-2 border-t border-border">
              ...
              <CheckRow
                ...
                label="Använd egendefinierad meddelandemall"
              />
              {customTemplate && (
                <textarea
                  rows={4}
      ...
      {/* ── SMS-avsändare och SMS-signatur ───────────────────────────────────── */}
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        <div>
          <h2 className="text-sm font-semibold text-foreground">SMS-avsändare och SMS-signatur</h2>
          <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
            Signatur används för alla automatiska utskick. Om du använder ett namn som avsändare
            kan mottagaren inte svara på meddelandet. SMS-avsändaren får inte vara ett mobilnummer.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            <label className="text-xs font-medium text-foreground">
              SMS-avsändare <span className="text-muted-foreground font-normal">(ej ÅÄÖ och max 11 tecken)</span>
            </label>
            <input
              type="text"
              value={smsAvsändare}
              onChange={e => setSmsAvsändare(e.target.value.replace(/[åäöÅÄÖ]/g, ''))}
              maxLength={11}
              className={inputCls}
            />
          </div>
          <div className="space-y-1">
            ...
            <input
              type="text"
              value={smsSignatur}
              onChange={e => setSmsSignatur(e.target.value)}
              maxLength={50}
              className={inputCls}
            />
          </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        ...
        <div className="space-y-3">
          <CheckRow
            checked={emailSmsStatus}
            onChange={setEmailSmsStatus}
            label="Dagligt e-post till företaget med status på skickade SMS-påminnelser"
          />
          <CheckRow
            checked={emailDailyStaff}
            onChange={setEmailDailyStaff}
            label="E-post till företaget med en översikt över medarbetarnas dagliga schema"
          />
          <CheckRow
            checked={emailReceipt}
            onChange={setEmailReceipt}
            label="Maila kvitto till skolan vid betalning"
          />
          <CheckRow
            checked={emailBookReq}
            onChange={setEmailBookReq}
            label="E-post till företaget för bokningsförfrågningar via TABSwebb"
          />
          <CheckRow
            checked={emailCopyInstr}
            onChange={setEmailCopyInstr}
            label="Kopia av e-postbekräftelse till utbildaren vid godkännande av bokning"
          />
          <CheckRow
            checked={emailCancelInstr}
            onChange={setEmailCancelInstr}
            label="Skicka e-post till utbildaren vid avbokning av körlektion eller kurs"
          />
        </div>
      ...
      {/* ── E-post: Misslyckad leverans ──────────────────────────────────────── */}
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        <h2 className="text-sm font-semibold text-foreground">Notifikationer - Misslyckad leverans</h2>
        <p className="text-xs text-muted-foreground leading-relaxed">
          Systemet skickar ut en notifikation till personalen när ett utgående meddelande
          inte kunnat levereras. Välj nedan vilken i personalen som ska få dessa notifikationer.
        </p>
        <select
          value={misslyckadStaff}
          onChange={e => setMisslyckadStaff(e.target.value)}
          className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm
                     focus:outline-none focus:ring-2 focus:ring-ring text-muted-foreground"
        >
