...
const LICENSE_GROUPS: LicenseGroup[] = [
  ...
  },
  {
    group: 'Traktor',
    licenses: [
      { code: 'T',         label: 'Behörighet T',      abbr: 'Tr',       webAlias: 'Trak',   webCat: '',                    catColor: '',       defaultOn: false },
    ],
  },
  {
    group: 'Snöskoter',
    licenses: [
      { code: 'S',         label: 'Behörighet S',      abbr: 'S',        webAlias: 'Snö',    webCat: '',                    catColor: '',       defaultOn: false },
    ],
  },
  {
    group: 'Yrkesförare',
...
// ─── UtbildningsbehörigheterPage ──────────────────────────────────────────────

export function UtbildningsbehörigheterPage() {
  const allCodes = LICENSE_GROUPS.flatMap(g => g.licenses).map(l => l.code);

  const [enabled, setEnabled] = useState<Set<string>>(
    () => new Set(LICENSE_GROUPS.flatMap(g => g.licenses).filter(l => l.defaultOn).map(l => l.code))
  );
  const [webAliases, setWebAliases] = useState<Record<string, string>>(
    () => Object.fromEntries(LICENSE_GROUPS.flatMap(g => g.licenses).map(l => [l.code, l.webAlias]))
  );
  const [editingCat, setEditingCat] = useState<string | null>(null);
  const [webCats, setWebCats]       = useState<Record<string, string>>(
    () => Object.fromEntries(LICENSE_GROUPS.flatMap(g => g.licenses).map(l => [l.code, l.webCat]))
  );
  const [catColors, setCatColors]   = useState<Record<string, string>>(
    () => Object.fromEntries(LICENSE_GROUPS.flatMap(g => g.licenses).map(l => [l.code, l.catColor]))
  );
  ...
  function toggleAll(checked: boolean) {
    setEnabled(checked ? new Set(allCodes) : new Set());
  }
  ...
  function startEditCat(code: string) {
    setCatInput(webCats[code] ?? '');
    setEditingCat(code);
  }

  function saveCat(code: string) {
    setWebCats(prev => ({ ...prev, [code]: catInput }));
    setEditingCat(null);
  }
    ...
    <div className="max-w-4xl space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card p-8 flex flex-col items-center text-center gap-2">
        ...
        <p className="text-sm text-muted-foreground">
          Hantera de utbildningsbehörigheter som tillhandahålls av trafikskolan.
        </p>
      </div>
            ...
            {LICENSE_GROUPS.map(({ group, licenses }) => (
              <>
                ...
                <tr key={`group-${group}`} className="bg-muted/10">
                  <td colSpan={5} className="px-4 py-2 text-xs font-bold text-foreground uppercase tracking-wide border-y border-border/60">
                    {group}
                  </td>
                </tr>
                ...
                {licenses.map(lt => {
                  ...
                  return (
                    <tr
                      ...
                      className={cn(
                        'transition-colors',
                        isOn ? 'hover:bg-accent/20' : 'opacity-50 hover:bg-accent/10'
                      )}
                    >
                      {/* Checkbox */}
                      <td className="px-4 py-2">
                        <input
                          type="checkbox"
                          checked={isOn}
                          onChange={() => toggleLicense(lt.code)}
                          className="rounded border-border accent-primary w-4 h-4"
                        />
                      </td>
                      ...
                      {/* Abbreviation (read-only display) */}
                      ...
                      <td className="px-4 py-2">
                        <input
                          type="text"
                          value={webAliases[lt.code] ?? ''}
                          onChange={e => setWebAliases(prev => ({ ...prev, [lt.code]: e.target.value }))}
                          className="w-full max-w-[120px] h-7 px-2 text-xs border border-border rounded bg-background text-foreground
                        ...
                        {editingCat === lt.code ? (
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              autoFocus
                              className="h-7 px-2 text-xs border border-primary rounded bg-background text-foreground
                                         focus:outline-none focus:ring-1 focus:ring-primary/40 w-32"
                            />
                            <button
                              type="button"
                              onClick={() => saveCat(lt.code)}
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        ...
        <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
          <input
            type="checkbox"
            checked={arkiverateori}
            onChange={e => setArkiverateori(e.target.checked)}
            className="rounded border-border accent-primary w-4 h-4"
          />
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        ...
        <p className="text-xs text-primary leading-relaxed">
          Fyll i det meddelande du vill skicka när en behörighet slutförts, t.ex. länk till enkät eller recension.
        </p>
