...
const errs = [];
page.on('response', async (res) => { if (res.status() >= 400 && !res.url().includes('notifications') && !res.url().includes('/instructors?')) errs.push(`[${res.status()}] ${res.request().method()} ${res.url()}`); });
...
await page.getByRole('textbox', { name: 'Förnamn *' }).fill('Sara');
await page.getByRole('textbox', { name: 'Efternamn *' }).fill('Svensson');
await page.getByRole('textbox', { name: 'E-postadress' }).fill('sara.svensson@example.test');

await page.getByRole('button', { name: 'Skapa' }).click();
await page.waitForTimeout(2500);
console.log('url:', page.url());
await page.screenshot({ path: `${SS}/12-student-created.png`, fullPage: true });
