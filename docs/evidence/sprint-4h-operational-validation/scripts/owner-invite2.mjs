...
try {
  ...
  await page.screenshot({ path: `${SS}/24-invite-dialog-open.png` });

  await page.locator('#inv_first_name').fill('Gustav');
  await page.locator('#inv_last_name').fill('Gästlärare');
  await page.locator('#inv_email').fill('pilot-validation-invited-staff@example.test');
  await page.screenshot({ path: `${SS}/24b-invite-dialog-filled.png` });
  ...
  await page.screenshot({ path: `${SS}/25-invite-error-state.png`, fullPage: true }).catch(()=>{});
}
