...
const errs = [];
page.on('response', async (res) => { if (res.status() >= 400 && !res.url().includes('notifications')) { let b=''; try{b=await res.text();}catch{}; errs.push(`[${res.status()}] ${res.request().method()} ${res.url()} :: ${b.slice(0,200)}`);} });
...
console.log('--- Bokningsschema ---');
await page.goto('http://localhost:5173/scheduling', { waitUntil: 'domcontentloaded' });
await page.waitForTimeout(2500);
console.log('url:', page.url());
await page.screenshot({ path: `${SS}/28-booking-calendar.png`, fullPage: true });
