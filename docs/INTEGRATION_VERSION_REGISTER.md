...
- **RLS:** unchanged.
- **Runtime payment resolution:** ~~unchanged~~ **corrected 2026-08-11, commit `b83e641846ca177c7be4bea33a8776dd4bc742e5`.** A follow-up Payment Provider Resolution Safety Check found that `payments/index.ts`, `student-portal/index.ts`, and `guardian-portal/index.ts` (a third call site not previously documented — staff-initiated Stripe links, student self-service checkout, and guardian self-service checkout respectively) all fell back to `Deno.env.get('STRIPE_SECRET_KEY')`/`NETS_SECRET_KEY` whenever an org had no credential in `settings`, with **no check on the pilot flag at all** — the "already satisfied" claim above was wrong. It was currently harmless only because those two env vars are not configured as platform secrets (confirmed via `supabase secrets list`, names only, no values read) — the moment either one ever were set, every not-configured tenant would have silently started processing payments through that account. The fallback has been removed at all three call sites: no tenant credential in `settings` now returns a clean `503 PROVIDER_NOT_CONFIGURED`/`Nets-betalning är inte konfigurerad`, never a platform account. `stripe-webhook`'s separate `STRIPE_WEBHOOK_SECRET` fallback (signature verification, not payment-credential selection) is unaffected and was not in scope.
- **Verification (2026-08-11):** both real tenants confirmed unchanged (`stripe_pilot_configuration`/`nets_pilot_configuration` still `true`, credentials still the shared sandbox copy) — both have a credential in `settings`, so this fix does not change their behavior at all, only that of any org with none. The new state-computation logic (from the first 2026-08-11 change, above) was replicated read-only against live data and correctly resolves both to `'pilot'` for both providers. `pnpm typecheck` clean; `deno check`/`deno lint` on all touched files show only the same pre-existing, unrelated findings already documented throughout this session (confirmed via `git stash` for each). Live bundle byte-identical to the built source for the chunk containing the new banner strings. Live authenticated write/payment test: **not attempted** in either change — no authenticated tenant session available, and doing so would require either a real Stripe/Nets key or bypassing existing live-validation, both out of scope/unsafe per explicit production-safety constraints throughout.
- **Rollback procedure:** `git revert b83e641846ca177c7be4bea33a8776dd4bc742e5` (fallback removal) and/or `git revert d1f0d054741c70771a1d187288069e8f9630479d` (pilot/owned distinction) — independently revertable, no data was altered by either, so rollback is a pure code reversal in both cases.
...
Same approved architectural decision, same commits, same verification as `payments-stripe` v2 above — see that entry for the full PLATFORM/TENANT/PILOT/business-model documentation and the fallback-removal follow-up, both of which apply identically to Nets. Nets-specific detail: `supabase/functions/nets-credentials/index.ts` now computes `nets_state` from `settings.nets_pilot_configuration`; saving either `nets_secret_key` or `nets_checkout_key` with a real value un-pilots the pair (they are tracked as one unit, not independently). No platform Nets webhook was implemented (out of scope — no platform Nets transaction flow exists to require one); the existing tenant-only `POST /nets-webhook/{organization_id}` route is unchanged.

- **Commits:** `d1f0d054741c70771a1d187288069e8f9630479d` (pilot/owned distinction), `b83e641846ca177c7be4bea33a8776dd4bc742e5` (removed the `Deno.env.get('NETS_SECRET_KEY')` fallback from `student-portal/index.ts`'s Nets checkout route).
...
- **Security incident & risk acceptance (2026-08-11):** the platform Roaring sandbox credential pair (set as `ROARING_CLIENT_ID`/`ROARING_CLIENT_SECRET` above) was inadvertently exposed in a shell-command transcript during this migration's implementation. Remediation was attempted: repo source, git history (`git log -S` pickaxe across all commits), documentation, and the built frontend bundle were all confirmed clean of the exposed values (never committed, never bundled); the Person Lookup config API was confirmed to never return credential fields. Rotation could not be completed — Roaring's dashboard (Developer → API keys → Sandbox keys) offers no self-service key regeneration, and their support channel could only reissue the identical pair or require a manual ticket. **Product Owner explicitly accepted this residual risk (2026-08-11)** on the basis that: (a) this is a sandbox-tier credential only, returning fictional data, with no path to real personnummer data; (b) the exposure was confined to a local terminal transcript, never reaching source control, documentation, or any deployed artifact; (c) Roaring provides no self-service rotation mechanism to act on even if desired. No further rotation action is planned unless Roaring's account tier changes (e.g. activation of production keys) or a new exposure vector is identified — a production-tier credential exposure would need to be treated as a new, higher-severity incident, not covered by this acceptance.
...
### v2 — tenant-owned (pilot safety hardening)

**Classification unchanged:** TENANT-OWNED, optional for V1.0, not required for pilot, partially implemented (OAuth complete, business-API synchronization dormant). This is a security/truthfulness hardening pass, not an architecture change — the shared-OAuth-application / per-tenant-token model from v1 is unmodified.

- **Commit:** `232f9ea`
- **Prompted by:** the 2026-08-11 "Fortnox Tenant-Owned Architecture Audit" (read-only), which found the OAuth `state` CSRF/tenant-binding check was conditionally skippable, the `queue_fortnox_sync` RPC had no execution restriction beyond RLS, and the tenant-facing UI presented dormant sync infrastructure (counters, retry, export lineage) as if it were operational.
- **Files changed:**
  - `supabase/functions/fortnox/index.ts` — `oauth/callback` now rejects the request outright if `state` is missing, malformed (not `{uuid}:{uuid}`), or bound to a different organization than the current session; only an exact-match valid state continues. PKCE (`code_verifier`) remains mandatory and unchanged. No change to client ID/secret source, OAuth provider, redirect handling, or token encryption.
  - `apps/web/src/modules/finance/routes/FortnoxCallbackPage.tsx` — mirrors the same fail-closed state check client-side (missing/mismatched state now rejected, not silently skipped) and always forwards `state` to the callback (previously optional).
  - `apps/web/src/modules/finance/routes/FortnoxPage.tsx` — removed the sync-counter cards, "Retry failed," and export-lineage table (none of it reflected real synchronization — see the audit's §10 finding that no code path ever calls Fortnox's business API). Replaced with an explicit connection-only view: connected/not-connected status plus a clear "Bokföringssynkronisering är inte tillgänglig ännu" ("accounting synchronization is not yet available") notice. OAuth connect/disconnect/refresh — all real, working functionality — is unchanged.
  - `apps/web/src/modules/settings/routes/ExternalServicesPage.tsx` (`FortnoxCard` only) — description corrected from implying active sync to stating the connection-only, sync-not-yet-available reality.
- **DB migrations:** `20260811130000_fortnox_queue_sync_rpc_hardening.sql` — `REVOKE EXECUTE ... FROM PUBLIC/anon/authenticated; GRANT EXECUTE ... TO service_role;` on `queue_fortnox_sync(uuid, text, uuid)`. Postgres grants EXECUTE to PUBLIC by default at function-creation time and no REVOKE had ever been issued for this function; live-verified post-migration via `information_schema.routine_privileges` that only `postgres` (owner) and `service_role` retain EXECUTE.
- **RLS:** unchanged — this was a function-execution-grant fix, not an RLS change. The four `fortnox_*_sync` tables' existing `organization_id = jwt.org_id` policies are untouched.
- **Config/secrets:** unchanged — same `FORTNOX_CLIENT_ID`/`FORTNOX_CLIENT_SECRET` platform-wide OAuth app registration as v1.
- **Deployment:** `supabase db push --linked` (migration applied and grants verified live), `supabase functions deploy fortnox --project-ref ulgsndzfksphquqakelq` (bare deploy, `verify_jwt: true` confirmed unchanged, version 74), frontend rebuilt and synced to Hostinger via FTP; live site confirmed `200` and the new `useFortnoxStatus` bundle chunk reachable.
- **Verification:** `pnpm typecheck` clean (8/8 packages). `deno check`/`deno lint` on `fortnox/index.ts` show only the same two pre-existing, unrelated findings already documented for every other integration this session (`credential-crypto.ts` overload errors, `no-import-prefix` on the untouched `esm.sh` import) — confirmed via `git stash`. `eslint` on all three touched frontend files: zero warnings or errors. OAuth state fail-closed logic verified by direct code-path tracing (deterministic, no external dependency): missing state → `MISSING_STATE` 400; malformed state (no colon, or a non-UUID segment) → `INVALID_STATE` 400; well-formed state for a different org → `INVALID_STATE` 400; well-formed state matching the current org → proceeds to token exchange. A live authenticated end-to-end call was not attempted — no test JWT/session is available in this environment, consistent with every other live-auth-gated verification this session; the RPC grant change was instead verified directly at its source of truth (`information_schema.routine_privileges`), which is stronger proof than an HTTP round-trip would have been.
- **Fortnox business synchronization:** confirmed NOT implemented by this change — no reference to `api.fortnox.se` was introduced (grep-verified against the full diff); only OAuth state validation, one RPC's execution grants, and UI copy were touched.
- **Rollback procedure:** `git revert 232f9ea` restores the conditional state check and the old sync-implying UI. To revert the RPC grant independently: `GRANT EXECUTE ON FUNCTION public.queue_fortnox_sync(uuid, text, uuid) TO authenticated;` in a new forward migration (the underlying function body is unchanged, so this is purely a grant reversal). No tenant data or existing connections were altered by this change.
