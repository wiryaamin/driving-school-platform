...
| 1.0 — Dunning Workflow Reliability (Action 6) | 2026-07-14 | This handbook, revised (this revision); Pilot Readiness Action Plan | Resolved the High-priority operational-reliability defect from the Pilot Readiness Assessment ("a silently-failing dunning/reminder dispatch," Action 0 row above): `event-worker/index.ts`'s `handleInvoiceReminderSent` — the handler for the `Invoice.ReminderSent` event both `advance_dunning_stage()` and `process_dunning_tick()` (`invoice_reminder_log`/`invoice_dunning_state`, unchanged Dunning SQL, `phase4b_dunning.sql`) already emit whenever a dunning stage is reached — was a pure logging stub (`// TODO: dispatch email/SMS/both based on action_type`), so **zero** dunning reminders were ever actually dispatched, while the append-only `invoice_reminder_log` and dunning state already recorded the stage as actioned. Fixed by making the handler enqueue a `Communication.Requested` event — mirroring `handleInvoiceIssued`'s already-proven, already-deployed pattern exactly (same `insert_outbox_event` RPC shape, same `p_causation_id` linkage, same student contact-info lookup) — which routes through the existing, unmodified `handleCommunicationRequested` (the platform's single dispatch point to `communication-worker/notify`, with its existing retry/dead-letter handling) using the `invoice_overdue` trigger_event, an **already-seeded** template and notification-rule row (`phase_a_notification_templates.sql`) that was simply never reachable before. `action_type = 'legal'` (an internal escalation, not a customer notification) is explicitly excluded from dispatch. No new notification system, template, schedule, or communication provider was introduced — every piece reused was already deployed and working for `Invoice.Issued`. **Verification note:** live end-to-end dispatch confirmation (an actual message reaching `communication-worker`) requires either the `WORKER_SECRET` used to invoke `event-worker` directly, or a naturally-occurring overdue invoice processed by a scheduled `process_dunning_tick()` cron run — neither was available in this session (no existing overdue invoices/dunning state in either test org, and `WORKER_SECRET` was deliberately not rotated, since doing so would risk breaking other already-configured consumers of that shared secret). Verified instead: `pnpm typecheck`/`lint`/`build` clean; the new code is structurally identical to the already-proven `handleInvoiceIssued` pattern; deployment succeeded with gateway `verify_jwt` confirmed unchanged (`false`, correct for this worker-secret-authenticated function). A live worker invocation to confirm actual dispatch is recommended as a follow-up before relying on this in a real pilot. No Finance, Invoice, Dunning, Communication, Worker, Event, Audit, RBAC, or multi-tenancy architecture change. | Approved |
| 1.0 — Deployment Safety Verification (Action 7) | 2026-07-14 | This handbook, revised (this revision); `scripts/verify-deployment-config.mjs`; `supabase/config.toml`; Pilot Readiness Action Plan | Implemented the automated Deployment Safety Verification recommended but never built (this Handbook, Architecture Governance Rule, "CI/CD recommendation (not yet implemented)" — now updated below) after the `verify_jwt` redeploy regression recurred a second time on `guardian-portal` during Action 2. `supabase/config.toml` is treated as the platform's existing approved deployment manifest (no new/duplicate manifest introduced) and was completed to give every one of the 59 deployed Edge Functions an explicit `[functions.<name>]` entry — 37 already had one; the remaining 22 previously had none, meaning a bare `supabase functions deploy <name>` would silently apply the platform's implicit default (`verify_jwt = true`) rather than any documented, reviewed value. The new script `scripts/verify-deployment-config.mjs` (`pnpm verify:deployment`) parses this manifest, calls the existing `supabase functions list --project-ref <ref> -o json` (the same command used manually throughout Actions 2-6) for live gateway state, and fails loudly (non-zero exit, per-function report) on any `verify_jwt` mismatch, inactive deployment, function present on one side but not the other, or function directory missing a manifest entry. Investigating the 22 previously-undeclared functions surfaced real, currently-live configuration drift, not a hypothetical risk: live-verified via `supabase functions list` and confirmed empirically with unauthenticated requests, 10 of the 59 deployed functions — `stripe-webhook`, `health`, `public-booking`, `public-catalog`, `public-enrollment`, `demo-requests`, `instructor-portal`, `communication-worker`, `instructor-ical`, `platform-bootstrap` — are live with `verify_jwt = true` despite requiring `false`, because their real callers (Stripe's webhook delivery, uptime monitors, anonymous public-site visitors, instructor self-service portal tokens, WORKER_SECRET worker-to-worker calls, and the pre-auth bootstrap flow) never hold a Supabase-signed JWT and are rejected at the gateway — confirmed live as `401 UNAUTHORIZED_NO_AUTH_HEADER` — before the function's own code, and its own auth check, ever runs. Notably, `communication-worker`'s drift means the `handleCommunicationRequested` dispatch path relied upon by Action 6's dunning-reminder fix (and by `Invoice.Issued` before it) may itself be gateway-blocked in production today — a pre-existing configuration gap unrelated to and undetected by Action 6's own verification, surfaced here as a direct consequence of building this tool. Per this Handbook's own 3-way classification rule (Section 5, Scope Freeze), this drift is newly-discovered work, out of Action 7's own scope (build a verifier, not remediate 10 unrelated live functions, and explicitly excluded by its DO NOT list — do NOT redeploy unrelated Edge Functions) and is logged as a Pilot Blocker candidate below (Section 12) for separate, explicit action rather than being silently fixed here. `config.toml` changes are declarative only — no Edge Function was redeployed and no live gateway configuration was altered as part of this action; the 37 already-explicit entries (all functions touched by Actions 2-6) showed zero drift, confirming those fixes hold. `pnpm typecheck`/`lint`/`build` unaffected (new script is a standalone Node file outside the pnpm workspace). No Edge Function code, Authentication, JWT handling, RBAC, Worker framework, CI/CD pipeline, or multi-tenancy architecture change. | Approved |
| 1.0 — Production Configuration Drift Remediation (Action 7A) | 2026-07-14 | This handbook, revised (this revision); Pilot Readiness Action Plan | Resolved the Pilot Blocker candidate logged by Action 7 (Section 12): the 10 live Edge Functions with `verify_jwt` drift (`stripe-webhook`, `health`, `public-booking`, `public-catalog`, `public-enrollment`, `demo-requests`, `instructor-portal`, `communication-worker`, `instructor-ical`, `platform-bootstrap`) were each redeployed with `supabase functions deploy <name> --project-ref ulgsndzfksphquqakelq --no-verify-jwt`, matching their already-correct `supabase/config.toml` declaration (`false`, set by Action 7) — the same reliable pattern already used for `bankid-auth`/`event-worker`/`guardian-portal`, since `config.toml`'s own `verify_jwt` value is `not reliably honored on deploy` (documented `config.toml` comment, confirmed again by this remediation: the value had already been `false` in `config.toml` since Action 7, yet all 10 functions were still live `true` until explicitly redeployed with the flag). Every affected function's source directory was confirmed clean (`git status`, matching committed `HEAD`) before deploying, so each redeploy pushed only the already-committed, already-live code — no application code, business logic, or unrelated function was touched. `pnpm verify:deployment` now reports **59/59 PASS, 0 FAIL** (previously 49/59). Empirical re-verification (live, unauthenticated requests, mirroring Action 7's own methodology) confirmed the gateway no longer rejects any of the 10 with `401 UNAUTHORIZED_NO_AUTH_HEADER`; each now reaches its own function code. Eight behave exactly as designed (`public-catalog` returns a real `200` catalog response; `public-booking`/`public-enrollment`/`demo-requests` return their own `422`/`400` input-validation errors on an empty test payload; `instructor-portal`/`communication-worker` return their own `401` on a missing portal token / `WORKER_SECRET`, i.e. their own auth check now runs, which was previously unreachable; `instructor-ical` returns its own `405` on an unsupported method; `platform-bootstrap` returns its own intentional fail-closed `503` once the bootstrap secret is unset). **Two newly-discovered, unrelated findings surfaced only because the gateway no longer masks them, logged below (Section 12) as new Pilot Blocker candidates and explicitly not fixed here** (out of Action 7A's scope — deployment-configuration remediation only, no application-code or business-logic change): `stripe-webhook` returns its own `503 {"error":"Webhook not configured"}` (an application-level secret/configuration gap, separate from `verify_jwt`); `health` returns its own `404 {"message":"Unknown health route: /health/live"}` (an internal route-matching detail in the function's own router). No Authentication, JWT handling, RBAC, Edge Function business logic, CI/CD, multi-tenancy, or deployment framework change. | Approved |
...
**CI/CD recommendation — implemented (Action 7, 2026-07-14):** `supabase/config.toml` is the approved deployment manifest, pairing every one of the 59 deployed Edge Functions with its approved `verify_jwt` value (all 59 now have an explicit `[functions.<name>]` entry; none rely on the platform's implicit default). `scripts/verify-deployment-config.mjs` (`pnpm verify:deployment`) reads the live configuration via `supabase functions list -o json`, compares it against this manifest, and exits non-zero with a per-function report on any mismatch, inactive deployment, or manifest/deployment inconsistency — checking actual deployed state rather than assuming deployment-tooling behavior, exactly as recommended. It is not yet wired into an automated CI/CD pipeline gate (no such pipeline exists yet for this repository); running it is currently a manual pre-Sprint-Closure step, consistent with the Edge Function Authentication Verification checklist above.
...
Action 7 finding (2026-07-14) - newly-discovered Pilot Blocker candidate, not yet actioned: building the Deployment Safety Verification tool surfaced 10 live Edge Functions with incorrect `verify_jwt` (`stripe-webhook`, `health`, `public-booking`, `public-catalog`, `public-enrollment`, `demo-requests`, `instructor-portal`, `communication-worker`, `instructor-ical`, `platform-bootstrap` — all live `true`, all confirmed by unauthenticated live request to require `false`). Most notably: Stripe webhook delivery (payment confirmation) and `communication-worker` (the dispatch target of Action 6's dunning-reminder fix and of `Invoice.Issued` notifications) are both gateway-blocked for their real callers. Per this Handbook's Scope Freeze classification rule, this is out of Action 7's own scope (a verification tool, not a remediation of 10 unrelated live functions - explicitly excluded by Action 7's own DO NOT list) and requires separate, explicit classification and action before Sprint Closure. `scripts/verify-deployment-config.mjs` (`pnpm verify:deployment`) reports all 10 today; no live configuration was altered as part of Action 7.

Action 7A update (2026-07-14): the finding above is **resolved** — all 10 functions redeployed with `--no-verify-jwt`, `pnpm verify:deployment` now reports 59/59 PASS, 0 FAIL, zero configuration drift. Resolving it surfaced two new, smaller, unrelated findings (not `verify_jwt`, not deployment configuration) logged here as new Pilot Blocker / Commercial Release Enhancement candidates for separate future action, per this Handbook's classification rule: (1) `stripe-webhook` returns its own `503 {"error":"Webhook not configured"}` on a live unauthenticated test call — an application-level secret/configuration gap (e.g. `STRIPE_WEBHOOK_SECRET` not set), meaning Stripe payment webhook delivery may still not be processing even though the gateway now admits the call; (2) `health` returns its own `404 {"message":"Unknown health route: /health/live"}` — an internal route-matching detail in the function's own router, meaning uptime monitors hitting `/health/live` may still not get a true liveness response even though the gateway now admits the call. Neither was fixed as part of Action 7A (explicitly out of scope: no application code or business logic change).
