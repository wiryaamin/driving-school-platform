...
|---|---|---|---|
| Undefined `ok()` function, 31 call sites in `compliance/index.ts` (Phase 6A/6B routes) | Outstanding defect (pre-existing, not introduced by PR-2) | **Medium** — any of the 31 routes throws `ReferenceError` if hit | High — narrowly-scoped, low-effort fix; should be its own small Production Readiness package |
